...
def create_starburst_tree(name, location, trunk_mat, spike_mat):
    trunk = add_cylinder(f"{name}_trunk", (location[0], location[1], location[2] + 0.7), 0.09, 1.4, trunk_mat)
    core = add_ico(f"{name}_core", (0.0, 0.0, 1.7), 0.34, (1.0, 1.0, 1.0), spike_mat, parent=trunk)
    directions = [
        (0.0, 0.0, 0.0),
...
def create_crystal_cluster(glass_mat, dark_mat, red_mat):
    ...
    for index, (loc, scl, rot) in enumerate(crystal_specs):
        shard = add_cone(f"Crystal_{index}", loc, 0.48, 1.72, glass_mat, rotation=rot, radius2=0.08, vertices=6)
        shard.scale = scl
...
def stylize_scene_objects():
    ...
    for obj in bpy.data.objects:
        ...
        if name.startswith("Tree_") and ("_crown" in name or "_accent" in name or "_core" in name):
            stylize_mesh(obj, bevel=0.01, bevel_segments=2, subdiv=1, weighted_normal=False)
            continue

        if name.startswith("Tree_") and "_spike_" in name:
            stylize_mesh(obj, bevel=0.005, bevel_segments=2, subdiv=0, weighted_normal=True)
            continue
...
def create_scene():
    ...
    cream = make_principled_material("Cream", (0.965, 0.952, 0.936, 1.0), roughness=0.88)
    charcoal = make_principled_material("Charcoal", (0.045, 0.04, 0.045, 1.0), roughness=0.34, metallic=0.22)
    graphite = make_principled_material("Graphite", (0.11, 0.105, 0.11, 1.0), roughness=0.4, metallic=0.16)
    red = make_principled_material("SignalRed", (0.56, 0.07, 0.09, 1.0), roughness=0.3, metallic=0.08)
    crimson = make_principled_material("Crimson", (0.76, 0.34, 0.3, 1.0), roughness=0.4, metallic=0.02)
    ...
    glass_red = make_glass_material("GlassRed", (0.34, 0.06, 0.08, 1.0), roughness=0.05, transmission=0.85, alpha=0.4)
    crystal = make_glass_material("CrystalGlass", (0.97, 0.95, 0.96, 1.0), roughness=0.015, transmission=1.0, alpha=0.18)
    ...
    create_tree("Tree_Right_Red", (7.92, -0.24, 0.0), charcoal, red, crimson)
    create_tree("Tree_Right_Black", (7.02, 0.82, 0.0), charcoal, graphite, charcoal)
    create_tree("Tree_Right_Front", (6.82, 1.82, 0.0), charcoal, crimson, red)
